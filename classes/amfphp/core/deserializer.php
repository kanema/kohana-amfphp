...
/**
 * Amfphp_Core_Deserializer takes the raw amf input stream and converts it PHP objects
 * representing the data.
 *
 * @package Amfphp_Core_Amf
 */
class Amfphp_Core_Deserializer {
    ...
    public function deserialize() {
        $this->deserializedPacket = new Amfphp_Core_Packet();
    ...
    protected function readHeaders() {
        ...
        //
            ...
            throw new Kohana_Exception("Malformed Amf Packet, connection may have dropped");
        }
        ...
        while ($this->headersLeftToProcess--) { // loop over all of the header elements
            ...
            $header = new Amfphp_Core_Header($name, $required, $content);
    ...
    protected function readMessages() {
        ...
        while ($this->messagesLeftToProcess--) { // loop over all of the Message elements
            ...
            $message = new Amfphp_Core_Message($target, $response, $data);
    ...
    public function readData($type) {
        switch ($type) {
            ...
            case 5: // null
                return null;
            case 6: // undefined
                return new Amfphp_Core_Types_Undefined();
            ...
            default: // unknown case
                throw new Kohana_Exception("Found unhandled type with code: $type");
                exit();
                break;
        }
        return $data;
    ...
    protected function readDouble() {
        ...
        $this->currentByte += 8;
        if (Amfphp_Core_Util::isSystemBigEndian()) {
            $bytes = strrev($bytes);
        }
    ...
    protected function readDate() {
        ...
        $date = new Amfphp_Core_Types_Date($ms);
        return $date;
    }

    /**
     *
     * @return Amfphp_Core_Types_Xml 
     */
    protected function readXml() {
        $str = $this->readLongUTF();
        return new Amfphp_Core_Types_Xml($str);
    }
     ...
     * to the object Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE.  This property will be overwritten if it existed within the class already.
     *
    ...
    public function readAmf3Data() {
        ...
        $this->deserializedPacket->amfVersion = Amfphp_Core_Constants::AMF3_ENCODING;
        $type = $this->readByte();
        switch ($type) {
            case 0x00 :
                return new Amfphp_Core_Types_Undefined();
            ...
            default:
                throw new Kohana_Exception("undefined Amf3 type encountered: " . $type);
        }
    }
    ...
    protected function readAmf3Date() {
        ...
        if (($firstInt & 0x01) == 0) {
            ...
            if ($firstInt >= count($this->storedObjects)) {
                throw new Kohana_Exception('Undefined date reference: ' . $firstInt);
                return false;
            }
    ...
    protected function readAmf3String() {
        ...
        if (($strref & 0x01) == 0) {
            ...
            if ($strref >= count($this->storedStrings)) {
                throw new Kohana_Exception('Undefined string reference: ' . $strref, E_USER_ERROR);
                return false;
            }
    ...
    protected function readAmf3Xml() {
        ...
        return new Amfphp_Core_Types_Xml($xml);
    }
    ...
    protected function readAmf3XmlDocument() {
        ...
        return new Amfphp_Core_Types_XmlDocument($xml);
    }
    ...
    protected function readAmf3ByteArray() {
        ...
        $handle = $handle >> 1;
        if ($inline) {
            $ba = new Amfphp_Core_Types_ByteArray($this->readBuffer($handle));
