...
class Amfphp_Core_Serializer {
    ...
    /**
     *
     * @param <Amfphp_Core_Packet> $packet
     */
    public function __construct(Amfphp_Core_Packet $packet) {
        $this->packet = $packet;
    ...
    protected function writeDouble($d) {
        ...
        if (Amfphp_Core_Util::isSystemBigEndian()) { // if we are a big-endian processor
            $r = strrev($b);
     ...
     */
    protected function writeXML(Amfphp_Core_Types_Xml $d) {
     ...
     *
     * @param Amfphp_Core_Types_Date $d The date value
     */
    protected function writeDate(Amfphp_Core_Types_Date $d) {
        $this->writeByte(0x0B); 
     ...
     * To accomplish this we just blanket grab all of the object vars with get_object_vars, minus the Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE field, whiuch is used as class name
     *
     * @param object $d The object to serialize the properties. The deserializer looks for Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE on this object and writes it as the class name.
     */
    protected function writeTypedObject($d) {
        ...
        $explicitTypeField = Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE;
        $className = $d->$explicitTypeField;
        if (!$className) {
            throw new Kohana_Exception(Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE . " not found on a object that is to be sent as typed. " . print_r($d, true));
        }
    ...
    protected function writeData($d) {
        if ($this->packet->amfVersion == Amfphp_Core_Constants::AMF3_ENCODING) { //amf3 data. This is most often, so it's has been moved to the top to be first
            $this->writeByte(0x11);
            ...
            $this->writeNull();
            return;
        } elseif (Amfphp_Core_Util::is_undefined($d)) { // undefined
            $this->writeUndefined();
            return;
        } elseif (is_array($d)) { // array
            $this->writeArrayOrObject($d);
            return;
        } elseif (Amfphp_Core_Util::is_date($d)) { // date
            $this->writeDate($d);
            return;
        } elseif (Amfphp_Core_Util::is_Xml ($d)) { // Xml (note, no XmlDoc in AMF0)
            $this->writeXML($d);
            return;
        } elseif (is_object($d)) {
            $explicitTypeField = Amfphp_Core_Constants::FIELD_EXPLICIT_TYPE;
        ...
        throw new Kohana_Exception("couldn't write data " . print_r($d));
    }
    ...
    protected function writeAmf3Data(& $d) {
        if (is_int($d)) { //int
            ...
            return;
        } elseif (Amfphp_Core_Util::is_byteArray($d)) { //byte array
            $this->writeAmf3ByteArray($d->data);
            return;
        } elseif (Amfphp_Core_Util::is_Xml ($d)) { // Xml
            $this->writeAmf3Xml($d);
            return;
        } elseif (Amfphp_Core_Util::is_XmlDocument ($d)) { // XmlDoc
        ...
        throw new Kohana_Exception("couldn't write object " . print_r($d, false));
    }
    ...
    protected function writeAmf3Xml(Amfphp_Core_Types_Xml $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x0B);
        $this->writeAmf3String($d);
    }

    protected function writeAmf3XmlDocument(Amfphp_Core_Types_XmlDocument $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x07);
        $this->writeAmf3String($d);
    }

    protected function writeAmf3Date(Amfphp_Core_Types_Date $d) {
        $this->writeByte(0x08);
        $this->writeAmf3Int(1);
        $this->writeDouble($d->timeStamp);
    }
    ...
    private function handleReference(&$obj, array &$references){
        ...
        if($key !== false){
            ...
            if($this->packet->amfVersion == Amfphp_Core_Constants::AMF0_ENCODING){
